...
function BalanceSheetVisual(){
  return <motion.div className="balance-sheet-visual" initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.7}}>
    <div className="visual-title"><FaChartBar/><span>Balance Sheet Snapshot</span><small>Balanced</small></div>
    <div className="balance-equation">
      <motion.div whileHover={{y:-5}}><span>Assets</span><strong>100%</strong><i style={{width:'100%'}}/></motion.div>
      <b>=</b>
      <motion.div whileHover={{y:-5}}><span>Liabilities</span><strong>42%</strong><i style={{width:'42%'}}/></motion.div>
      <b>+</b>
      <motion.div whileHover={{y:-5}}><span>Equity</span><strong>58%</strong><i style={{width:'58%'}}/></motion.div>
    </div>
    <div className="balance-footer"><FaCheckCircle/> Accounting equation reconciled</div>
  </motion.div>
}

function InvoiceVisual(){
  return <motion.div className="invoice-visual" initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.7,delay:.1}}>
    <motion.div className="invoice-paper" animate={{y:[0,-7,0],rotate:[0,-1,0]}} transition={{duration:4,repeat:Infinity}}>
      <div className="invoice-top"><FaFileInvoiceDollar/><div><b>INVOICE</b><small>#FIN-2026-018</small></div><span>PAID</span></div>
      {[['Professional services','75%'],['Tax & compliance','52%'],['Reporting support','64%']].map(([label,width],i)=><div className="invoice-line" key={label}><span>{label}</span><motion.i initial={{width:0}} whileInView={{width}} viewport={{once:true}} transition={{duration:.8,delay:.25+i*.15}}/></div>)}
      <div className="invoice-total"><span>Total processed</span><strong>100%</strong></div>
    </motion.div>
    <div className="invoice-flow"><span>Prepared</span><b>→</b><span>Approved</span><b>→</b><span>Paid</span></div>
  </motion.div>
}
...
function App(){
  ...
  return <div className="app">
    ...
    <main>
      <section id="home" className="hero section-bg hero-bg">
        ...
        <div className="container hero-grid">
          ...
          <motion.div className="hero-portrait" initial={{opacity:0,x:35}} animate={{opacity:1,x:0}} transition={{duration:.9}}>
            ...
            <div className="hero-experience"><strong><Counter value={9} suffix="+"/></strong><span>Years of experience</span></div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section finance-visuals-bg"><div className="container"><div className="section-heading" data-aos="fade-up"><p className="eyebrow">Finance Visuals</p><h2 className="section-title">Accounting concepts presented with motion and clarity.</h2></div><div className="finance-visuals-grid"><BalanceSheetVisual/><InvoiceVisual/></div></div></section>
